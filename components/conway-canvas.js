...
class GameOfLife {
    ...
    for (let x = 0; x < this.col; x++) {
      for (let y = 0; y < this.row; y++) {
        let numAlive =
          ...
          this._isAlive(x, y + 1)
        if (numAlive == 2) {
          this.gameObjects[this._gridToIndex(x, y)].next =
            this.gameObjects[this._gridToIndex(x, y)].alive
        } else if (numAlive == 3) {
  ...
  _isAlive(x, y) {
    ...
    return this.gameObjects[
      this._gridToIndex((x + this.col) % this.col, (y + this.row) % this.row)
    ].alive
      ? 1
      : 0
  }
  ...
  draw(color1, color2) {
    ...
    for (let x = 0; x < this.col; x++) {
      for (let y = 0; y < this.row; y++) {
        this.gameObjects[this._gridToIndex(x, y)].draw(
          this._gradient(
            (((x + 1) / this.col) ** 2 + ((y + 1) / this.row) ** 2) ** 0.5,
            color1,
            color2
          )
        )
